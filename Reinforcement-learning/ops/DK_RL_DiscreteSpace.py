...
class AC_ICM_Module():
    # Actor-Critic On-Policy Gradient with ICM
    def __init__(self,Actor_net,Critic_net,device=None,using_entropy=False):
        ...
        self.icm = ICM.ICM_Model(state_dim=4,action_dim=2);
    ...
    def set_ActorOptimizer(self,optimizer=None):
        if(optimizer==None):
            self.Actor_optimizer = torch.optim.Adam(list(self.Actor_net.parameters()) + list(self.icm.parameters()));
        else:
    ...
    def get_policy_action(self,state,action_num=None):
        ...
        output = self.Actor_net(state).detach();
        ...
        return numpy_action,m.probs.cpu().numpy();
